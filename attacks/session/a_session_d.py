"""Phase 7 concrete attack for session boundary, STRIDE letter D (Denial of Service).
...
class SessionDAttack(Attack):
    """STRIDE row for the session boundary, letter D (Denial of Service).
    ...
    async def execute(self, ctx: AttackContext) -> AttackResult:
        """Run the session/D attack and return the result."""
