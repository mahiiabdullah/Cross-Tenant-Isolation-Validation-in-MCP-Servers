"""Phase 7 concrete attack for session boundary, STRIDE letter R (Repudiation).
...
class SessionRAttack(Attack):
    """STRIDE row for the session boundary, letter R (Repudiation).
    ...
    async def execute(self, ctx: AttackContext) -> AttackResult:
        """Run the session/R attack and return the result."""
